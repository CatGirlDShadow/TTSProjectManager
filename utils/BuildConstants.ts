...
export function ProcessReversedConstants(reversedConstants: Map<string, ConstantInfo>, content: string){
    for (const constantValue of reversedConstants.keys()){
        if (reversedConstants.get(constantValue).type == "string")
            content = content.replaceAll(constantValue, `{{%${reversedConstants.get(constantValue).name}%}}`);
        else
            content = content.replaceAll(constantValue, `"{{%${reversedConstants.get(constantValue).name}%}}"`);
    }
    return content;
